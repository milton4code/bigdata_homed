...
import cn.ipanel.common.{CluserProperties, Constant}
...
object KafkaOffsetManager {
  ...
  private val zkClient = new ZkClient(CluserProperties.ZOOKEEPER_URL)
   def readOffsets( zkPath: String, topic: String): Option[Map[TopicAndPartition, Long]] = {
  ...
  def saveOffsets(zkHosts:String, zkPath: String, rdd: RDD[_]): Unit = {
