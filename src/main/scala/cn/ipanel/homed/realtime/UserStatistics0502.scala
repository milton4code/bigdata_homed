...
//      runlogSream.checkpoint(Durations.seconds(duration * 10))
//
...
//
//
//  def foreachRdd(runLogDStream: DStream[Log], threads: Int, area_type: String): Unit = {
//    runLogDStream.foreachRDD(event => {
//      val sqlContext: SQLContext = SQLContext.getOrCreate(event.sparkContext)
//      import sqlContext.implicits._
//      event.toDF().registerTempTable("t_event")
//
//      //      event.toDF().selectExpr("da as event_da").show()
//      val logDF = sqlContext.sql(
//        """select device_id, DA, play_type,first(device_type) device_type,
//          |first( province_id) province_id,
//          |first(city_id) city_id,
//          |first(region_id) region_id,
//          |first(report_time) report_time,
//          |max(receive_time) receive_time
//          |from t_event
//          |group by device_id,DA,play_type""".stripMargin)
//
...
//        process(logDF, sqlContext, threads, area_type)
...
//          , "city_id", "region_id", "play_type", "report_time", " receive_time")
//
//        cacheUserToRedis(df)
//        cacheToMysql(df)
//      }
...
//        buffer.append(CachUser(device_id, DA, paras(0), paras(1), paras(2), paras(3), paras(4), paras(5).toLong, paras(6).toLong))
//      })
//
//
//      println("redisToDataFrame buffer is :"+buffer.isEmpty)
//      sqlContext.createDataFrame(buffer)
//    }*/
//
//  private def redisToDataFrame(redisResponse: Map[String, String], sqlContext: SQLContext): DataFrame = {
//    /*
//    case class CachUser(device_id: Long = 0L, DA: Long = 0L, play_type: String = "", device_type: String = "", province_id: String = "", city_id: String = "", region_id: String = "",
//                        report_time: Long = 0L, receive_time: Long = 0L)
//     */
//
...
//    })
//    println("redisToDataFrame buffer is :" + buffer.isEmpty)
//    //    sqlContext.createDataFrame(buffer,schema)
