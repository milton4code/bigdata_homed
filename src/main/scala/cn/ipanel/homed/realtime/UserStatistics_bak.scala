...
//
//case class tmpLog(key_word: String, DA: Long, access_token: String, device_id: String, device_type: String, play_type: String, play_time: Long, report_time: Long, receive_time: Long)
//
//object UserStatistics_bak {
...
//      //      println("=======_logGroupDF=======" + (e1 - s1) + " ms")
//
//      //      _logGroupDF.foreach(x=>{
//      //        logger.error("user:" + x.getAs[Long]("DA")
//      //          + ",report_time:" + DateUtils.unixTimeToDate( x.getAs[Long]("report_time"),"yyyy-MM-dd HH:mm:ss")
//      //          + " ,playType:" + x.getAs[String]("play_type") +",status :" + x.getAs[Boolean]("status")
//      //        )
//      //      })
//
//
...
//
//  //  //过滤有效日志
//  //  def filterLog(kafkaStream: InputDStream[(String, String)], regionCodeBr: Broadcast[String]): DStream[Log3] = kafkaStream.transform(rdd => {
//  //
//  //    val regionCode = regionCodeBr.value
//  //    rdd.flatMap(x=>{
//  //      val splits = x._2.split("- \\[INFO\\] -")
//  //
//  //      val logInfo = splits(1)
//  //      val reportTime:Long = getReportTime(splits(0))
//  //      val receiveTime:Long = System.currentTimeMillis()/1000
//  //      val keyWord = getKeyWord(splits(1))
//  //
//  //      val map = stringToMap(logInfo.substring(logInfo.indexOf(",")+1))
//  //      val accessToken = getToken(map("URI"))
//  //
//  ////      val ob = tmpLog(keyWord,map("DA").toLong,accessToken, map("DeviceID"),map("DeviceType"),map("ProgramMethod"),
//  ////                       map("PlayS").toLong,reportTime,receiveTime)
//  //
//  //      val json = new JSONObject()
//  //      json.put("key_word",keyWord)
//  //      json.put("DA",map("DA").toLong)
//  //      json.put("access_token",accessToken)
//  //      json.put("device_id", map("DeviceID"))
//  //      json.put("device_type", map("DeviceType"))
//  //      json.put("play_type", map("ProgramMethod"))
//  //      json.put("play_time", map("PlayS").toLong)
//  //      json.put("report_time",reportTime)
//  //      json.put("receive_time",receiveTime)
//  //
//  //      Some(json)
//  //
//  //    }).filter(_.getString("key_word").contains("StatisticsVideoPlay"))
//  //      .map(x => {
//  //      var regionIdStr = TokenParser.getRegionString(x.getString("access_token"))
//  //      if (Constant.EMPTY_CODE.equals(regionIdStr)) {
//  //        regionIdStr = regionCode
//  //      }
//  //
...
//  //      val cityId = regionIdStr.substring(0, 4) + "00"
//  //      val regionId = regionIdStr.substring(0, 6)
//  //      val statuts = if (LogConstant.videoSuccess.equals(x.getString("key_word"))) LogConstant.SUCCESS else LogConstant.FINISH
//  //
//  //      Log3(x.getLong("DA"), 0L,
//  //        x.getString("device_type"), x.getString("device_id"),
//  //        x.getLong("report_time"), x.getLong("receive_time"),
//  //        "", proviceId, cityId, regionId, x.getString("play_type"),
//  //        statuts, x.getLong("play_time"))
//  //    }).filter(_.DA == 55313240)
//  //  })
//  //
//  //  def stringToMap(logInfo: String): mutable.Map[String, String] = {
//  //    val map = new scalaMap[String, String]()
//  //    logInfo.split(Constant.SPLIT).filter(_.length > 0).foreach(word => {
